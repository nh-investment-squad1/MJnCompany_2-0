...
```bash
/MJ-plan "사용자 인증 시스템 (이메일+비밀번호, JWT)"
/MJ-plan --lang typescript "장바구니 도메인"
/MJ-plan --output docs/plans "결제 처리 유스케이스"
/MJ-plan --lang python --output plans "이메일 발송 서비스"
```
...
자세한 실행 프로토콜은 `domains/MJ-plan-v1/skills/MJ-plan/SKILL.md`를 참조하세요.
각 에이전트의 동작 정의는 `domains/MJ-plan-v1/agents/*.md` 파일에 명시되어 있습니다.
