...
4. 직접 오케스트레이션할 것인가, mj-smart-run에 위임할 것인가
...
설치 후 `/mj-ceo`를 다시 실행하거나, 지금 자동 설치를 진행할 수도 있습니다.
...
입력: "with mj-clarify,deep-research: 요구사항 정리 후 리서치"
파싱: partners=["mj-clarify","deep-research"], task="요구사항 정리 후 리서치"
...
| "요구사항 불명확" / "scope 정의" / "뭘 만들어야" | mj-clarify | Pre |
...
# mj-clarify (CS 시리즈 내부 파트너)
CLARIFY_SKILL=$(ls -d "$BASE/mj-clarify-v"* 2>/dev/null | sort -V | tail -1)
CLARIFY_SKILL="$CLARIFY_SKILL/skills/mj-clarify/SKILL.md"
...
| 상황 | 신호 | 권장 조치 |
|------|------|-----------|
| 이전 mj-ceo 실행 결과가 컨텍스트에 쌓여 있음 | 도메인 리포트, 도구 출력 누적 | `/compact` 권장 후 진행 |
...
if [ -n "$CMUX_SOCKET_PATH" ]; then
  cmux set-status "mj-ceo" "running" --icon "gear"
...
LATEST_SMARTRUN=$(ls -d "$BASE/mj-smart-run"* 2>/dev/null | sort -V | tail -1)
```
...
| "요구사항 불명확" / "scope 먼저" | mj-clarify | Pre | clarify → 재추정 → A/B/C |
...
if [ -n "$CMUX_SOCKET_PATH" ]; then
  ...
  cmux set-status "mj-ceo" "done" --icon "checkmark"
fi
```
...
트리거 있음 → 리포트 끝에:
```
💡 버전업 제안: `/mj-experiencing version-up all` 로 오늘 패턴을 노하우로 저장하세요.
```
